...
export abstract class Entry {
  ...
  public get zoteroFile(): string {
    ...
    console.log(pdfPaths[0].substring(index + 8, index + 9));
    if (pdfPaths[0].substring(index + 8, index + 9) == "\\") {
      var Paths = pdfPaths[0].substring(index + 9, index + 9 + 8);
    } else {
      var Paths = pdfPaths[0].substring(index + 8, index + 8 + 8);
    }
